...
let song = new Audio("assets/Sqrl%20sound.mp3");
...
); // Added difficulty buttons selection
...
function startGame() {
  song.volume = 0.5; // set the volume to 50%
...
function setDelay(difficulty) {
  ...
  if (difficulty === "hard") {
    ...
    return `Please pick easy, normal, or hard difficulty.`;
  }
}
...
function showAndHide(hole, delay) {
  ...
  const timeoutID = setTimeout(() => {
    ...
    if (time > 0) {
      ...
      timerDisplay.textContent = time; //Updates the timer display
      showUp(); //continue game
    } else {
      gameOver();
    }
...
function clearScore() {
  ...
  points = 0;
  score.textContent = points;
  console.log("Score cleared", points);
  return points;
}
